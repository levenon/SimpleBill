...
@interface SBPayVC ()<UITableViewDelegate, UITextFieldDelegate, SBUserSelectorVCDelegate, SBPayWaySelectorVCDelegate, SBBillKindSelectorVCDelegate>

@property (weak, nonatomic) IBOutlet UITableViewCell *evtbvcUserName;

@property (weak, nonatomic) IBOutlet UITableViewCell *evtbvcBillKind;
...
@property (strong, nonatomic) SBBillKind *evBillKind;

@end

@implementation SBPayVC

- (void)loadView{
    [super loadView];

    [self setTitle:NSLocalizedString(@"label.expend", @"Expend")];
...
- (void)setEvBill:(SBBill *)evBill{
    if (_evBill != evBill) {
        ...
        [self setEvBillKind:[evBill billKind]];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch ([indexPath row]) {
        ...
        case 1:
        {
            SBBillKindSelectorVC *etUserSelectorVC = [[SBBillKindSelectorVC alloc] init];
            [etUserSelectorVC setEvDelegate:self];
...
#pragma mark - SBBillKindSelectorVCDelegate

- (void)epBillKindSelector:(SBBillKindSelectorVC*)selector billKind:(SBBillKind*)billKind;{

    [self setEvBillKind:billKind];
    [[[self evtbvcBillKind] detailTextLabel] setText:ntoe([billKind name])];
}
...
- (void)_efConfigSubviews;{
    ...
    if ([self evEditType] != SBEditTypeAdd) {
        ...
        [[[self evtbvcBillKind] detailTextLabel] setText:ntodefault([[[self evBill] billKind] name], NSLocalizedString(@"label.none", @"None"))];
    ...
    if ([self evEditType] == SBEditTypeOnlyPreview) {
        ...
        [[self evtbvcBillKind] setAccessoryType:UITableViewCellAccessoryNone];
...
- (BOOL)_efSave;{
    ...
    if (![self evBillKind]) {
        ...
        [MBProgressHUD showWithStatus:NSLocalizedString(@"label.please.select.category", @"Please select expend category!")];
        return NO;
    }
    ...
    [etBill setBillKind:[self evBillKind]];
    [etBill setType:[NSNumber numberWithInteger:SBBillTypeOutput]];
